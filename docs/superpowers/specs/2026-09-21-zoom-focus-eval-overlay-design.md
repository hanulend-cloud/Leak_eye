...
- **AF 리전 좌표계 해석이 기기별로 달라 탭 위치와 실제 포커스 지점이 어긋날 수 있음 — 최종 리뷰에서 구체적 원인 후보를 특정함:** `configureTransform()`은 센서가 가로(landscape)로 읽히고 화면은 세로(portrait)라서 뷰-버퍼 rect를 서로 바꿔 매핑하는 방식(가로/세로 스왑)으로 별도 회전 없이 프리뷰를 올바르게 보여준다(주석 참고). 반면 `TapFocusMapper.mapTapToAfRegion()`은 `tapX/viewW`, `tapY/viewH`를 축 교환 없이 그대로 크롭 영역에 선형 매핑한다 — 프리뷰와 같은 축 교환이 AF 리전 계산에는 반영되지 않았을 가능성이 높다. 실기기 확인 시 **가장 먼저** 확인할 항목: 탭한 화면 위치와 실제 포커스가 맞는 지점이 90도 정도 어긋나 보이는지 확인하고, 어긋나면 `TapFocusMapper` 호출 전 tapX/tapY(또는 viewW/viewH)를 서로 바꿔서 넘기는 보정이 필요할 수 있음.
